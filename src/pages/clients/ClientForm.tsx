...
const ClientForm = () => {
  ...
  // Watch tier value to display correct quota and rate limit
  ...
  // Function to get tier details based on selected tier
  const getTierDetails = (tierValue: string) => {
    const selectedTier = CLIENT_TIERS.find((tier) => tier.value === tierValue);
    return {
      quota: selectedTier?.quota.toLocaleString() || '10,000',
      rateLimit: selectedTier?.rateLimit || '50',
    };
  };

  // Get tier details for the currently selected tier
  const tierDetails = getTierDetails(watchedTier);
                  ...
                  <div className="space-y-2">
                    <p className="text-sm text-gray-500 flex justify-between">
                      ...
                      <span className="font-medium">
                        {tierDetails.quota} requests
                      </span>
                    </p>
                    <p className="text-sm text-gray-500 flex justify-between">
                      <span>Rate Limit:</span>
                      <span className="font-medium">
                        {tierDetails.rateLimit} req/min
                      </span>
                    </p>
                  </div>
                </div>
